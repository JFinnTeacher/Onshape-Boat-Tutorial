...
Looking closer at @overview, #link(<overview>)[Box 1] contains the the "Graphics Area" in which you view and edit your model. Select a part of the model to see a corresponding "Feature" highlighted in the "Feature List" in #link(<overview>)[Box 2]. The "Feature List" keeps a history of every operation performed, allowing you to go back and make changes to earlier states of the model. To add to features to the "Feature List," select actions like #icon("sketch") `Sketch` and #icon("extrude") `Extrude` on the "Toolbar" in #link(<overview>)[Box 3]. Selecting a tool will open a "Dialog Window" as seen in Box 4 and houses the parameters to modify the current operation. To see how tools affect your design, click on the "View Finder" in #link(<overview>)[Box 5] to change your view angle. 
